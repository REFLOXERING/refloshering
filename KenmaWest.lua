...
getgenv().Move1 = char.Humanoid.AnimationPlayed:Connect(function(v)
    if v.Animation.AnimationId == "rbxassetid://15434683426" then
        -- Play the startup/miss animation below TimePosition 1.5
        if v.TimePosition < 1.5 then
            local MissAnim = Instance.new("Animation")
            MissAnim.AnimationId = "rbxassetid://121382760132081"
            local missAnimTrack = char.Humanoid:LoadAnimation(MissAnim)
            missAnimTrack.Priority = Enum.AnimationPriority.Action4
            missAnimTrack:Play()

            -- Stop the miss animation after it plays for 1.3 seconds
            task.delay(1.15, function()
                if missAnimTrack.IsPlaying then
                    missAnimTrack:Stop()
                end
            end)

            -- Wait until original animation reaches 1.5 to play combo
            repeat task.wait() until v.TimePosition >= 1.75

            local ComboAnim1 = Instance.new("Animation")
            ComboAnim1.AnimationId = "rbxassetid://132609447121299"
            local combo1Track = char.Humanoid:LoadAnimation(ComboAnim1)
            combo1Track.Priority = Enum.AnimationPriority.Action4
            combo1Track:Play()

            -- Play the first combo for 0.6 seconds
            task.delay(0.6, function()
                combo1Track:Stop()

                local ComboAnim2 = Instance.new("Animation")
                ComboAnim2.AnimationId = "rbxassetid://93701672887866"
                local combo2Track = char.Humanoid:LoadAnimation(ComboAnim2)
                combo2Track.Priority = Enum.AnimationPriority.Action4
                combo2Track:Play()

                -- Allow the second combo to continue until the original animation ends
...
if v.Animation.AnimationId == "rbxassetid://15432272268" then
local Anim = Instance.new("Animation")
Anim.AnimationId = "rbxassetid://111717847758922"
...
if v.Animation.AnimationId == "rbxassetid://14771588355" then
local Anim = Instance.new("Animation")
Anim.AnimationId = "rbxassetid://100645622099987"
local k = char.Humanoid:LoadAnimation(Anim)
k.Priority = Enum.AnimationPriority.Action4
k:Play()
k.TimePosition = 2.75
...
local function loadClothing(character)
    -- Remove existing shirt and pants only
    ...
    newPants.PantsTemplate = 'rbxassetid://13842297231'
    newPants.Name = 'Pants'
    newPants.Parent = character

    -- Add new Shirt
    local newShirt = Instance.new("Shirt")
    newShirt.ShirtTemplate = 'rbxassetid://13842296590'
    newShirt.Name = 'Shirt'
    newShirt.Parent = character
end

local function onCharacterAdded(character)
    wait(getgenv().Time or 0)  -- Default to 0 if Time is nil
    -- Don't call loadAccessories if you don't want to add accessories
    loadClothing(character)
end

-- Connect when the character is added
game.Players.LocalPlayer.CharacterAdded:Connect(onCharacterAdded)

-- Optionally apply immediately if character already loaded
...
--// Constants
local ANIM_ID = "rbxassetid://14382045837"
local BASE_SPEED = 40
local MAX_SPEED = 320
local ACCEL_RATE = 60
local TWEEN_TIME = 0.05
local IDLE_STOP_DELAY = 0.1 -- how fast it cuts speed/anim on stop
...
local sprintTrack = humanoid:LoadAnimation(anim)
sprintTrack.Looped = true
sprintTrack:AdjustSpeed(4.5)
...
local function tweenSpeed(speed)
	TweenService:Create(humanoid, TweenInfo.new(TWEEN_TIME), { WalkSpeed = speed }):Play()
end
...
local function stopSuperSpeed()
	...
	isRunning = false
	forceSpeed = false
	tweenSpeed(16) -- Reset to default
end

--// Detect External Speed Interference & Override
humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
	if forceSpeed and humanoid.WalkSpeed < currentSpeed - 1 then
		-- Some script tried to slow us down — override it
		humanoid.WalkSpeed = currentSpeed
	end
end)

--// Main Super Speed Loop
RunService.RenderStepped:Connect(function(dt)
	...
	if moving then
		...
		if isRunning then
			-- Short delay to allow micro stops (optional)
			task.delay(IDLE_STOP_DELAY, function()
...
player.CharacterAdded:Connect(function(newChar)
	...
	sprintTrack = humanoid:LoadAnimation(anim)
	sprintTrack.Looped = true
	sprintTrack:AdjustSpeed(4.5)
...
local url = "https://github.com/REFLOXERING/refloshering/raw/refs/heads/main/Wally_West_Theme_My_Ordinary_Life_x_I_Got_No_Time.mp3" -- example
local fileName = "IsthatWallyWest.mp3"  -- You can name the file however you want
...
sound.Volume = 0.85  -- Set volume to desired level
